...
void Core::CreateLogicalDevice()
{
  ...
  vkGetDeviceQueue(mLogicalDevice, mFamilyPair.second, 0, &mPresentQueue);
}
...
void Core::CreateSwapChain()
{
  ...
  bool familiesNotEqual = mFamilyPair.first != mFamilyPair.second;
  ...
  VkSwapchainCreateInfoKHR createInfo = {
      ...
      .imageSharingMode = familiesNotEqual ? VK_SHARING_MODE_CONCURRENT : VK_SHARING_MODE_EXCLUSIVE,
      .queueFamilyIndexCount = familiesNotEqual ? 2u : 0u,
      .pQueueFamilyIndices = familiesNotEqual ? queueFamilyIndices : nullptr,
  ...
  // create the imageViews for the swapchain
...
void Core::CreateFrameBuffers()
{
  ...
  for (u64 i = 0; i < mSwapChainImageViews.size(); i++) {
    ...
    assert(vkCreateFramebuffer(mLogicalDevice, &framebufferInfo, nullptr, &mSwapChainFrameBuffers[i]) == VK_SUCCESS);
  }
}
...
void Core::CreateCommandPool()
{
  ...
  VkCommandPoolCreateInfo poolInfo = {
      ...
      .queueFamilyIndex = (u32)mFamilyPair.first, // *queueFamilyIndices.mGraphicsFamily;
  };

  assert(vkCreateCommandPool(mLogicalDevice, &poolInfo, nullptr, &mCommandPool) == VK_SUCCESS);
}
